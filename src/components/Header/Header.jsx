    ...
    <>
      <nav className="flex items-center h-20 text-white justify-between px-2 py-3 bg-[#2738F5]">
        ...
        <ul className="hidden md:flex gap-4">
          ...
          <button type="button"
            className="hidden md:block border-solid border-zinc-100 border py-4 px-4 rounded-md  hover:opacity-80" onClick={handleOpenModal1}> Regístrate
          </button>

          {
            openModal && (
              <div id='modal-backdrop' className='absolute inset-0 bg-[#252424cc] w-screen h-full' onClick={(e) => {
                ...
                <div onClick={handleCloseModal} className="fixed top-0 left-0 w-full h-full bg-opacity-50 z-50">
...

          {
            openModal1 && (
              <div id='modal-backdrop' className='absolute inset-0 bg-[#252424cc] w-screen h-full' onClick={(e) => {
                if (e.target.id === 'modal-backdrop') { setOpenModal1(false) }
              }}>

                <div onClick={handleCloseModal1} className="fixed top-0 left-0 w-full h-full bg-opacity-50 z-50">
                  <div onClick={(e) => e.stopPropagation()} className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[28rem] h-[27rem] border-stone-500 bg-sky-100 rounded-md">
                    <div className="flex flex-col items-center justify-center h-full text-black">
                      ...
                      <h1 className="static text-2xl font-semibold -mt-5 mb-5">Regístrate</h1>
