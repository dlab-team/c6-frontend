...
const EducationalBackgroundForm = () => {
    ...
    const {data: institutionData, isLoading: institutionLoading} = useFetch('http://localhost:3000/api/institutions');
    const institutions = institutionData && institutionData.map((institution) => ({
        value: institution.name,
        label: institution.name
    }));
    const {data: institutionTypeData, isLoading: institutionTypeLoading} = useFetch('http://localhost:3000/api/institutionstype');
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4 mx-8 md:mx-20">
                ...
                <div className="form-group md:col-span-2 required position-relative">
                    ...
                    <div className="form-group md:col-span-2 required mt-10 position-relative">
                        ...
                        type="text"
                        required
                        className={`form-control w-80 md:w-5/6 h-10 px-4 rounded-md border-2 border-custom-color mt-5 mb-5 bg-custom-color ${errors.career1 ? 'is-invalid' : ''}`} 
                        {...register('career1')} 
                        />
                    ...
                    <div className="form-group required position-relative">
                        ...
                        <div>
                            <Select
                                ...
                                className={`form-control basic-multi-select w-5/6 h-10 rounded-md mt-5 mb-5 text-custom-color ${errors.institution1 ? 'is-invalid' : ''}`} 
                     ...
                     <div className="form-group required position-relative">
                        ...
                        <div>
                            <Select
                                ...
                                name="institution2"
                                options={institutions}
                                className={`form-control basic-multi-select w-5/6 h-10 rounded-md mt-5 mb-5 text-custom-color ${errors.institution2 ? 'is-invalid' : ''}`} 
                ...
                {/* div nivel de ingles */}
                <div className="form-group required mt-5 position-relative">
                    <label className="control-label position-absolute mb-5 text-custom-color">¿Cuál es tu nivel de inglés?</label>
                    <select 
                        required
